...
type
  ...
  public
    ...
    procedure AddBytes(const AField: string; const ABytes: TBytes;
      const AFileName: string = ''; const AContentType: string = '');
    ...
    /// <summary>Return a form data boundary string</summary>
  ...
  private
    FItems: TList<TItem>;
    FSubject: string;
    function GetNames(AIndex: Integer): string; inline;
    function GetValue(const AName: string): string; inline;
...
{$REGION 'MIME CONST'}
  ...
  AddType('map', 'application/json', TMimeTypes.TKind.Text); // do not localize
  ...
  AddType('json', 'application/vnd.embarcadero.firedac+json', TMimeTypes.TKind.Text); // do not localize
...
begin
  LType := AType.Trim.ToLower;
  if LType = '' then
    raise EArgumentOutOfRangeException.CreateRes(@SMimeTypeEmpty);
  ...
  begin
    ...
    if AIgnoreDups then
      Exit;
    raise EListError.CreateRes(@SMimeDuplicateType);
  end;
...
begin
  ...
  if LName = '' then
    raise EArgumentOutOfRangeException.CreateRes(@SMimeValueEmpty);
  if (AWeight < 0) or (AWeight > 1) then
    raise EArgumentOutOfRangeException.CreateRes(@SMimeWeightOutOfRange);
