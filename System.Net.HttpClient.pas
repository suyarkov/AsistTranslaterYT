...
// -------------------------------------------------------------------------------- //
  ...
  protected
    ...
    procedure DoSendDataProgress(AContentLength: Int64; AWriteCount: Int64; var AAbort: Boolean; AAllowCancel: Boolean);
...
// -------------------------------------------------------------------------------- //
  ...
  public
    ...
    property AutomaticDecompression: THTTPCompressionMethods
      read FAutomaticDecompression write FAutomaticDecompression
      default [];
...
begin
  Result := HttpToDate(AStrDate, False);
end;
...
initialization
  TURLStream.RegisterSyncReqExecutor('HTTP',
    function (AClient: TURLClient; const ARequest: IURLRequest;
      const AContentStream: TStream; const AHeaders: TNetHeaders): IURLRequest
    begin
      Result := IURLRequest(THTTPClient(AClient).Execute(IHTTPRequest(ARequest), AContentStream, AHeaders));
    end);
  TURLStream.RegisterSyncReqExecutor('HTTPS',
    function (AClient: TURLClient; const ARequest: IURLRequest;
      const AContentStream: TStream; const AHeaders: TNetHeaders): IURLRequest
    begin
      Result := IURLRequest(THTTPClient(AClient).Execute(IHTTPRequest(ARequest), AContentStream, AHeaders));
...
finalization
  TURLStream.UnRegisterSyncReqExecutor('HTTP');
  TURLStream.UnRegisterSyncReqExecutor('HTTPS');
end.
